...
import type { AppUser, RoleMeta, RoleName } from "@/data/seed";
 ...
 * inline role-select per user (see Usuarios.tsx for the rationale) —
 * dispatching the same UPDATE_USER_ROLE action the legacy drop handler used.
 */
export function RoleCard({
  ...
  canManage: boolean;
  onAddClick: () => void;
  onRoleChange: (userName: string, role: RoleName) => void;
}) {
      ...
      <CardContent className="flex flex-1 flex-col gap-2 pt-0">
        ...
        {users.map((u) => (
          ...
          >
            ...
            {canManage ? (
              <Select value={u.role} onValueChange={(v) => onRoleChange(u.name, v as RoleName)}>
