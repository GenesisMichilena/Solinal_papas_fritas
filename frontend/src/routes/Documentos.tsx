...
export default function Documentos() {
  ...
  const docList = useMemo(() => {
    ...
    if (isLector) {
      list = list.filter((d) => d.estado === "Aprobado");
    }
    return list.filter((d) => {
      const matchEstado =
        estadoFilter === "all" ||
        (estadoFilter === "Vencido" ? d.vencido : d.estado === estadoFilter);
      ...
      return matchEstado && matchType && matchNorma && matchSearch;
    });
  }, [state.documents, isLector, estadoFilter, typeFilter, normaFilter, search]);
