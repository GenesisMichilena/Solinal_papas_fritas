...
import { demoCredentials, findCredential, type DemoCredential } from "@/features/auth/credentials";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Separator } from "@/components/ui/separator";
import { TwoFactorDialog } from "@/components/layout/TwoFactorDialog";
...
const googleCredential = demoCredentials.find((c) => c.role === "Administrador")!;

/** Standalone login screen — the app's entry point. Simulated auth (no real
 * backend): each of the 5 roles has its own dedicated demo credential, and
 * logging in as one is the only way to change which role/user is active —
 * the old in-app "cycle role" control was removed from the Topbar. */
export default function LoginPage() {
  ...
  const [pendingLogin, setPendingLogin] = useState<{ credential: DemoCredential; method: string } | null>(
    null,
  );
  ...
  function finalizeLogin(credential: DemoCredential, method: string) {
    dispatch({ type: "LOGIN", payload: { user: credential.user, role: credential.role } });
    dispatch({
      type: "ADD_AUDIT_LOG",
      payload: { action: `Inicio de sesión (${method}) como ${credential.user}` },
    });
    toast.success(`Bienvenido, ${credential.user}.`);
    navigate("/", { replace: true });
  }

  function attemptLogin(credential: DemoCredential, method: string) {
    if (state.config.twoFactorEnabled) {
      setPendingLogin({ credential, method });
      setTwoFactorOpen(true);
      return;
    }
    finalizeLogin(credential, method);
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!credential) {
      toast.error("Credenciales inválidas. Verifica el correo y la contraseña, o elige un usuario de prueba.");
      return;
    }
    attemptLogin(credential, "credenciales");
  }
  ...
  return (
    <div className="grid min-h-svh grid-cols-1 lg:grid-cols-2">
            ...
            <div className="grid grid-cols-1 gap-1.5">
              {demoCredentials.map((credential) => (
                <button
                  key={credential.role}
                  type="button"
                  onClick={() => attemptLogin(credential, "acceso rápido")}
                  className="flex items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-white/10"
                >
                  <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">
                    {initialsOf(credential.user)}
                  </span>
                  <span className="min-w-0 flex-1 leading-tight">
                    <span className="block truncate text-xs font-semibold">{credential.role}</span>
                    <span className="block truncate text-[11px] text-navy-foreground/50">
                      {credential.user} · {credential.email}
                    </span>
                  </span>
                </button>
              ))}
      ...
      <TwoFactorDialog
        ...
        onVerified={() => {
          if (pendingLogin) finalizeLogin(pendingLogin.credential, pendingLogin.method);
